...
import { Plus, Home, MapPin, DollarSign, Eye, Edit, Trash2, Bed } from 'lucide-react';
...
function MyPropertiesPage() {
  ...
  const fetchProperties = async () => {
    try {
      const result = await api.get('/properties/my-properties');
      ...
      setError(err.response?.data?.message || err.message || 'Error al cargar las propiedades');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (propertyId) => {
    ...
    try {
      ...
      alert(err.response?.data?.message || err.message || 'Error al eliminar propiedad');
    }
  };
        ...
        {properties.length === 0 && !error ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {properties.map((property) => {
              ...
              const title = property.title || property.hotelName || 'Sin título';
              const city = property.city || property.addressCity || '';
              const country = property.country || property.addressCountry || '';
              const rooms = property.rooms || [];
              const firstRoomImage = rooms.length > 0 && rooms[0].images?.length > 0 ? rooms[0].images[0] : null;
              const minPrice = rooms.length > 0 ? Math.min(...rooms.map(r => parseFloat(r.pricePerNight))) : property.basePrice;
              const maxCapacity = rooms.length > 0 ? Math.max(...rooms.map(r => r.guestCapacity)) : property.guests;

              return (
                <div key={property.id} className="bg-white rounded-xl shadow-sm overflow-hidden hover:shadow-md transition">
                  {/* Image */}
                  <div className="relative h-48 bg-gray-200">
                    {firstRoomImage || (property.images && property.images.length > 0) ? (
                      <img
                        src={firstRoomImage || property.images[0]}
                        alt={title}
                  ...
                  <div className="p-4">
                    ...
                    {rooms.length > 0 ? (
                      <div className="flex items-center gap-2 text-sm text-gray-600 mb-4">
                        <span>Hasta {maxCapacity} huéspedes</span>
                        <span>•</span>
                        <span>{rooms.length} habitación(es)</span>
                      </div>
                    ) : (
                      <div className="flex items-center gap-2 text-sm text-gray-600 mb-4">
                        <span>{property.guests || 0} huéspedes</span>
                        <span>•</span>
                    ...
                    <div className="space-y-2 pt-4 border-t">
                      ...
                      {/* Room Management Button (for new format properties) */}
                      {rooms.length > 0 && (
                        <Link
                          to={`/host/properties/${property.id}/rooms`}
                          className="w-full flex items-center justify-center gap-2 px-3 py-2 bg-primary/10 text-primary border border-primary/20 rounded-lg hover:bg-primary/20 text-sm font-medium transition"
                        >
                          <Bed size={16} />
                          Gestionar habitaciones ({rooms.length})
                        </Link>
                      )}
                    </div>
                  </div>
